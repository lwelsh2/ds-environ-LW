...
```{r all-time-webs}
all_time_webs <- vector("list", ncol(sp_occ))

# Loop through the time periods (columns of sp_occ)
for (time_period in 1:ncol(sp_occ)) {
  # Extract the species present in the current time period
  present_species <- sp_occ[, time_period]
  
  # Zero out rows and columns for species not present in the time period
  adj_matrix_time <- sweep(adj_matrix, 1, present_species, `*`)
  adj_matrix_time <- sweep(adj_matrix_time, 2, present_species, `*`)
  
  # Store the filtered adjacency matrix (network) in the list
  all_time_webs[[time_period]] <- adj_matrix_time
}
head(all_time_webs[[5]], 5)
...
```{r graphs}
# Initialize an empty list to store the igraph objects for each time period
igraph_networks <- vector("list", length(all_time_webs))

# Convert each adjacency matrix to an igraph object
for (time_period in 1:length(all_time_webs)) {
  # Convert the adjacency matrix to a directed, weighted igraph object
  igraph_networks[[time_period]] <- graph_from_adjacency_matrix(
    all_time_webs[[time_period]], 
    mode = "directed", 
    diag = FALSE, 
    weighted = TRUE
  )
...
```{r plot-g1}
carnivore_colors <- ifelse(carnivores == 1, "red", "green")

# Plot the first network (choose a time period, e.g., 1st)
plot_g1 <- function() {
  # Choose the first time period network (or any other you prefer)
  g1 <- igraph_networks[[1]]
  
  # Plot with different colors for predators and prey
  plot(g1, 
       vertex.color = carnivore_colors, 
       vertex.size = 15, 
       vertex.label.cex = 0.8, 
       edge.arrow.size = 0.5, 
       main = "Network for Time Period 1")
}

# Plot the second network (choose the 5th time period)
plot_g2 <- function() {
  # Choose the fifth time period network
  g2 <- igraph_networks[[5]]
  ...
  plot(g2, 
       ...
       vertex.label.cex = 0.8, 
       edge.arrow.size = 0.5, 
       main = "Network for Time Period 5")
}

# Plot the third network (choose the 10th time period)
plot_g3 <- function() {
  # Choose the tenth time period network
  g3 <- igraph_networks[[10]]
  plot(g3, 
       vertex.color = carnivore_colors, 
       vertex.size = 15, 
       vertex.label.cex = 0.8, 
       edge.arrow.size = 0.5, 
       main = "Network for Time Period 10")
}
