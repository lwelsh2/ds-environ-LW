...
```{r}
terwilliger_rast <- rast("/Users/lwelsh/Desktop/ds-environ-LW/5-OR-fires/soil-burn-severity/2018_terwilliger_sbs/SoilSeverity.tif")
beachiecrek_rast <- rast("/Users/lwelsh/Desktop/ds-environ-LW/5-OR-fires/soil-burn-severity/2020_beachiecreek_sbs/BeachieCreek_SBS_final.tif")
holidayfarm_rast <- rast("/Users/lwelsh/Desktop/ds-environ-LW/5-OR-fires/soil-burn-severity/2020_holidayfarm_sbs/HolidayFarm_SBS_final.tif")
```
...
summary(values(beachiecrek_rast))
...
beachiecrek_df <- as.data.frame(beachiecrek_rast, xy = TRUE)
ggplot() +
    geom_raster(data = beachiecrek_df , aes(x = x, y = y, fill = Layer_1)) +
    scale_fill_brewer(palette = "Spectral", direction=-1)
...
crs(beachiecrek_rast, proj = TRUE)
...
- ANSWER the same? :terwilliger and beachiecrek are the same The same? all of them use m

b. What about the resolution of each raster?

```{r, resolution}
res(terwilliger_rast)
res(beachiecrek_rast)
res(holidayfarm_rast)
```

- ANSWER resolution: Holiday: 20 20 Beachie: 20 20 Terwilliger: 30 30
- ANSWER the same? : The same? Holiday and Beachie are the same, Terwilliger is different
...
minmax(beachiecrek_rast)

min(values(beachiecrek_rast))

max(values(beachiecrek_rast))
...
```{r}
beachiecrek_rast[beachiecrek_rast > 4] <- NA
summary(values(beachiecrek_rast))
```
...
beachiecrek_rast = project(beachiecrek_rast, crs(holidayfarm_rast))
crs(beachiecrek_rast, proj = TRUE) == crs(holidayfarm_rast, proj = TRUE)
```
...
```{r}
template_rast <- holidayfarm_rast
terwilliger_rast_20 <- resample(terwilliger_rast, template_rast, method = "bilinear")

terwilliger_rast <- project(terwilliger_rast, crs(holidayfarm_rast))
...
terwilliger_df$SoilBurnSe <- factor(terwilliger_df$SoilBurnSe)
...
```{r oreco}
oregon_ecoregions <- st_read("/Users/lwelsh/Desktop/ds-environ-LW/5-OR-fires/OR-ecoregions/Ecoregions_OregonConservationStrategy.shp")
...
```{r or-crs}
st_crs(oregon_ecoregions) == st_crs(holidayfarm_rast)

oregon_ecoregions <- st_transform(oregon_ecoregions, crs(holidayfarm_rast))
st_crs(oregon_ecoregions) == st_crs(holidayfarm_rast)
```
...
all_fires_df <- rbind(beachiecrek_df, holidayfarm_df, terwilliger_df)

ggplot(all_fires_df, aes(x = as.factor(Layer_1), y = (..count..)/sum(..count..), fill = as.factor(Layer_1))) +
  geom_bar(position = "dodge") +
  scale_fill_brewer(palette = "Spectral", direction = -1) +
  labs(x = "Severity", y = "Proportion", title = "Proportion of Fire Severity Categories")
...
ANSWER: Low and Moderate are the most frequent, with moderate significantly higher than low. The reading discussed how burned areas were already at higher risk of reburn, this could be shown in the data as moderate and low burn sites could be reburning
